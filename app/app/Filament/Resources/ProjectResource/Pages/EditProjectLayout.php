...
class EditProjectLayout extends Page
{
    ...
    {
        if (! in_array($type, ['round', 'square'], true)) {
            $type = 'round';
        }
        ...
        $created = [];
        $width = $isSquare ? 96 : 92;
        $height = $isSquare ? 96 : 92;
        $gap = 28;
        $totalWidth = ($count * $width) + (($count - 1) * $gap);
        $startX = max(120, 1320 - $totalWidth + ($width / 2));
        $startY = 800;
        ...
        for ($index = 0; $index < $count; $index++) {
            ...
            $table = $this->currentSeatingPlan->tables()->create([
                'name' => 'Table ' . $order,
                'center_x' => $startX + ($index * ($width + $gap)),
                'center_y' => $startY,
                'rotation' => 0,
                'table_type' => $isSquare ? 'square' : 'round',
                'primary_dimension' => $width,
                'secondary_dimension' => $height,
                'seats_total' => $isSquare ? null : $seats,
                'seats_by_side_json' => $isSquare ? [
        ...
        foreach ($data['tables'] as $tableData) {
            ...
            ProjectTable::query()
                ...
                ->update([
                    ...
                    'primary_dimension' => $tableData['primary_dimension'],
                    'secondary_dimension' => $tableData['secondary_dimension'] ?? $tableData['primary_dimension'],
                    'seats_total' => $isRound ? (int) ($tableData['seats_total'] ?? 0) : null,
                    'seats_by_side_json' => $isRound ? null : ($tableData['seats_by_side_json'] ?? ['top' => 0, 'right' => 0, 'bottom' => 0, 'left' => 0]),
                ]);
        }
    ...
    public function duplicateTable(int $tableId): void
    {
        ...
        $nextOrder = ((int) $this->currentSeatingPlan->tables()->max('sort_order')) + 1;
        $seatMargin = 72;
        $nextCenterX = min(1350, (float) $source->center_x + (float) $source->primary_dimension + $seatMargin);

        $this->currentSeatingPlan->tables()->create([
            'name' => 'Table ' . $nextOrder,
